...
{
    public class 판매자APIService : JwtTokenAPIService
    {
        ...
        public async Task<Read판매자DTO> Create판매자(Create판매자DTO 판매자DTO)
        {
            ReadyBearerToken();
            var response = await _httpClient.PostAsJsonAsync("api/판매자", 판매자DTO);
            response.EnsureSuccessStatusCode();
            var created판매자DTO = await response.Content.ReadFromJsonAsync<Read판매자DTO>();
            return created판매자DTO;
        }

        public async Task Update판매자(string id, Update판매자DTO updated판매자DTO)
        {
            ReadyBearerToken();
            var response = await _httpClient.PutAsJsonAsync($"api/판매자/{id}", updated판매자DTO);
            response.EnsureSuccessStatusCode();
        }
