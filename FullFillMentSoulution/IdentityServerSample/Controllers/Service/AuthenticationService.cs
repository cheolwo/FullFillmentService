...
namespace IdentityServerSample.Service
{
        ...
        public async Task Login(LoginModel loginModel)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(token))
                {
                    ...
                    await ((CustomAuthenticationStateProvider)_authenticationStateProvider).SetAuthenticatedUser(claims, token);
                }
                else
                {
                    // 로그인 실패 처리
                ...
                if (!string.IsNullOrEmpty(expiration) && long.TryParse(expiration, out var expirationUnixTime))
                {
                    ...
                    if (timeRemaining <= _tokenRefreshThresholdSeconds)
                    {
                        ...
                        if (!string.IsNullOrEmpty(refreshedToken))
                        {
                            ...
                            // 사용자 정보와 권한 정보를 포함하여 인증 상태 설정
                            await ((CustomAuthenticationStateProvider)_authenticationStateProvider).SetAuthenticatedUser(refreshedClaims, refreshedToken);
                        }
                        else
                        {
                            // 토큰 갱신 실패 처리
    ...
    public class CustomAuthenticationStateProvider : AuthenticationStateProvider
    {
        ...
        public async Task SetAuthenticatedUser(List<Claim> claims, string token)
        {
