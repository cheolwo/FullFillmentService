...
namespace WarehouseCommon.APIService
{
    public class 출고상품APIService
    {
        ...
        public async Task<List<출고상품>> GetAll출고상품()
        {
            var response = await _httpClient.GetAsync("api/출고상품");
            response.EnsureSuccessStatusCode();
            var 출고상품List = await response.Content.ReadFromJsonAsync<List<출고상품>>();
            return 출고상품List;
        }

        public async Task<출고상품> Get출고상품ById(string id)
        {
            var response = await _httpClient.GetAsync($"api/출고상품/{id}");
            if (response.IsSuccessStatusCode)
            {
                var 출고상품 = await response.Content.ReadFromJsonAsync<출고상품>();
                return 출고상품;
            }
        ...
        public async Task<출고상품> Create출고상품(출고상품 출고상품)
        {
            var response = await _httpClient.PostAsJsonAsync("api/출고상품", 출고상품);
            response.EnsureSuccessStatusCode();
            var created출고상품 = await response.Content.ReadFromJsonAsync<출고상품>();
            return created출고상품;
        }

        public async Task<출고상품> Update출고상품(string id, 출고상품 updated출고상품)
        {
        ...
        public async Task<List<출고상품>> Get출고상품ListBy창고Id(string 창고Id)
        {
        ...
        public async Task<List<출고상품>> Get출고상품ListBy창고상품Id(string 창고상품Id)
        {
        ...
        public async Task<List<출고상품>> Get출고상품ListBy입고상품Id(string 입고상품Id)
        {
        ...
        public async Task<List<출고상품>> Get출고상품ListBy적재상품Id(string 적재상품Id)
        {
